...
Если объект является примитивом, то достаточно объявить его volatile. Все записи в это поле будут видны читающим потокам.
Для сложных объектов безопасно публикуется только ссылка, но не внутреннее состояние.
